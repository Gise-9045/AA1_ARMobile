...
{
    /// <summary>
    /// Onboarding goal to be achieved as part of the <see cref="CustomGoalManager"/>.
    /// </summary>
    public struct CustomGoal
    {
        /// <summary>
        /// Goal state this goal represents.
        /// </summary>
        public CustomGoalManager.OnboardingGoals CurrentGoal;
        ...
        /// <summary>
        /// Creates a new Goal with the specified <see cref="CustomGoalManager.OnboardingGoals"/>.
        /// </summary>
        /// <param name="goal">The <see cref="CustomGoalManager.OnboardingGoals"/> state to assign to this Goal.</param>
        public CustomGoal(CustomGoalManager.OnboardingGoals goal)
        {
            CurrentGoal = goal;
            Completed = false;
        }
    ...
    /// <summary>
    /// The CustomGoalManager cycles through a list of Goals, each representing
    /// an <see cref="CustomGoalManager.OnboardingGoals"/> state to be completed by the user.
    /// </summary>
    public class CustomGoalManager : MonoBehaviour
    {
        /// <summary>
        /// State representation for the onboarding goals for the CustomGoalManager.
        /// </summary>
        ...
        [SerializeField]
        ARTemplateMenuManagerModified m_MenuManager;

        /// <summary>
        /// The AR Template Menu Manager object to enable once the greeting prompt is dismissed.
        /// </summary>
        public ARTemplateMenuManagerModified menuManager
        {
            get => m_MenuManager;
        ...
        Queue<CustomGoal> m_OnboardingGoals;
        Coroutine m_CurrentCoroutine;
        CustomGoal m_CurrentGoal;
        ...
        public void StartCoaching()
        {
            ...
            m_OnboardingGoals = new Queue<CustomGoal>();

            if (!m_AllGoalsFinished)
            {
                var findSurfaceGoal = new CustomGoal(OnboardingGoals.FindSurfaces);
                m_OnboardingGoals.Enqueue(findSurfaceGoal);
            }

            int startingStep = m_AllGoalsFinished ? 1 : 0;

            var tapSurfaceGoal = new CustomGoal(OnboardingGoals.TapSurface);
            var translateHintsGoal = new CustomGoal(OnboardingGoals.Hints);
            var scaleHintsGoal = new CustomGoal(OnboardingGoals.Scale);
            var rotateHintsGoal = new CustomGoal(OnboardingGoals.Hints);
